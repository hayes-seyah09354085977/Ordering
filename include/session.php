...
function admin_product_exists($pid,$q){
    ...
    $flag=0;
    for($i=0;$i<$max;$i++){
      if($pid==$_SESSION['admin_gcCart'][$i]['productID']){
          if($q>0  && $q<=999){
            # code...
            $flag=1;
 ...
 function admin_addtocart($pid,$product,$price,$q,$subtotal){
    if($pid<1 or $q<1) return;
    if($q<1) return;
    ...
    if(is_array($_SESSION['admin_gcCart'])){
      ...
      $max=count($_SESSION['admin_gcCart']);
      $_SESSION['admin_gcCart'][$max]['productID']=$pid;
     ...
     $_SESSION['admin_gcCart']=array();
      $_SESSION['admin_gcCart'][0]['productID']=$pid;
