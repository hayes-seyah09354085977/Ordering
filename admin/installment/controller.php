...
switch ($action) {
	...
	function doInsert(){
		...
		if(isset($_POST['save'])){
			...
			$product->planname	= $_POST['plan']; 
			$product->percentageid	= $_POST['percent'];
			$product->months	= $_POST['mos'];  
			// $product->DateExpire		=  @$DateExpire;
			$product->create(); 
					message("New Installment created successfully!", "success");
					redirect("index.php");
 ...
 function checkExistingProduct($productName){
	...
	if(isset($_POST['save'])){
		...
		$product = New Installments(); 
			...
			message("Installment Save!", "success");
	...
	function doDelete(){
		global $mydb;
			$id = 	$_GET['id'];
			$product = New Installment();
	 		$product->delete($id);
			message("Installment already Deleted!","success");
			redirect('index.php');
	}
