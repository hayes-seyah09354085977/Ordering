              ...
              if (!empty($_SESSION['gcCart'])){  
                $count_cart = count($_SESSION['gcCart']); 
                    for ($i=0; $i < $count_cart  ; $i++) { 
                      // print_r($_SESSION['gcCart']);
                        	echo'<tr> 
                    				<td>'.$_SESSION['gcCart'][$i]['product'].'|</td>
                    				<td>'.$_SESSION['gcCart'][$i]['price'].'</td>
                    				<td><input style="height:25px;width:50px; border:none;" type="number" name="qty" id="'.$_SESSION['gcCart'][$i]['productID'].'qty" required class="cartqty" min="1" max="'.$_SESSION['pqty'].'" data-id="'.$_SESSION['gcCart'][$i]['productID'].'"  value="'.$_SESSION['gcCart'][$i]['qty'].'" autocomplete="off" /> </td>
